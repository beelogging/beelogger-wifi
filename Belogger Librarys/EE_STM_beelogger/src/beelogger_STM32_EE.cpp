...
*
* 21.04.2020 R.Schick
*  create beelogger_STM32_EE.cpp
*  declaration of I2C-Address extern
*  isPresent rewritten
*
* 22.11.2021 R.Schick
*  I2C Clock set to 100k
*
* 16.03.2022
*   add function end()
*
...
#include "beelogger_STM32_EE.h"
#define min(a,b) ((a)<(b)?(a):(b))


beelogger_STM32_EE::beelogger_STM32_EE(int addr)
{
  _i2c_addr = addr;
  Wire.begin();
...
void beelogger_STM32_EE::end(void){
  Wire.end();
}
...
uint8_t beelogger_STM32_EE::isPresent(void)      // check if the device is present
{
  char data[5] = "test";
  char dat[5] = "  ";
  WriteStr(0,data);
  ReadStr(0,dat,5);
  if((data[0]==dat[0])&&(data[1]==dat[1])) return(false);
  return(true);  // present: return = 0
}

int beelogger_STM32_EE::ReadMem(int iAddr, char Buf[], int iCnt)
{
  int iRead=0, iBytes;
  while (iCnt>0) {
...
int beelogger_STM32_EE::ReadStr(int iAddr, char Buf[], int iCnt)
{
  int iRead=0, iBytes;
  char c;
  while (iCnt>0) {
...
uint8_t beelogger_STM32_EE::WriteMem(int iAddr, uint8_t iVal)
{
  uint8_t iRC=0;
  Wire.beginTransmission(_i2c_addr);
#if ARDUINO >= 100
...
uint8_t beelogger_STM32_EE::WriteMem(int iAddr, const char *pBuf, int iCnt)
{
...
uint8_t beelogger_STM32_EE::WriteStr(int iAddr, const char *pBuf)
{
  uint8_t iRC=0;
