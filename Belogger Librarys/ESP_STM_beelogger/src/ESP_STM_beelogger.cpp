...
bool C_WLAN_ESP01::init(int baudrate)
{
  ...
  while (!Serial2); // Wait for serial port open
...
void C_WLAN_ESP01::send(const char* data)
{
  if (espState == ESP_CONNECTED){
    if (data == 0x00) {
      Serial2.write("\\0");  	// transmission of data start
  	  espState = ESP_WLAN_READY;
    }
    else {
      Serial2.print(data);
      Serial2.flush(); 		 	// Software Serial has only a 64byte buffer,  
 #if DEB_send // Debug 
        Serial.print(data);
        Serial.flush();
#endif
...
void C_WLAN_ESP01::firmware(unsigned long timeout)
{
  ...
  strcat(buffer,"\r\n");
  Serial2.print(buffer);		// firmware Information
  Serial2.flush();		 
