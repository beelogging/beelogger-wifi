/*************************************************************************
  ...
  V 1.0.2 fix reading signal level

  Version 1.0.3 01122021 R.Schick
     add delay when purging data

  Version 1.0.4 21022022 R.Schick
          CGPADDR response

  Version 1.0.5 15062022 R.Schick
          modify SIM7600E Init()
...
#define try_count 10
...
#if DEB_data
  Serial.println(F("SIM7600 15-06-2022"));
  Serial.flush();
#endif

  pinMode(GSM_RX, INPUT); // Turn pullup off 	pinMode(INPUT_CAPTURE_PIN, INPUT_PULLUP);
  delay(5000); // wait for SIM7600E coming up
  int i = 0;
  do {
    if (sendCommand("AT", 4000)) {
      break;
    }
    i++;
  }
  ...
  while (i < try_count);
  if (i ==  try_count) return (false);
  if(sendCommand("AT+CFUN=1", timeout)) return(true);// SIM Card on
  return false;
}
